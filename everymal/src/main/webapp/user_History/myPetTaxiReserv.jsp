  ...
  <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %> <!-- core라이브러리를 c라는 접두사를 따와서 쓰겠다 -->
    ...
    <% request.setCharacterEncoding("utf-8"); %>
      ...
      <head>
        ...
        <link rel="stylesheet" href="${contextPath}/css/myPetTaxiReserv.css" />
                        ...
                        <c:forEach var="rec" items="${taxiRecent}">
                          <div class="reservBox">
                            <a style="display: flex"
                              href="${contextPath}/history/viewTaxiRes.do?tx_res_code=${rec.ttx_res_code}">
                              <img src="${contextPath}/taxiImgdown.do?tx_code=${rec.ttx_code}&tx_img=${rec.tx_img}"
                                alt="">
                              <div class="r_smallBox" style="display: flex; flex-direction: column">
                                <p class="r_info_text"><span class="r_infotit">예약일</span>${rec.ttx_reserv_day}</p>
                                <p class="r_info_text"><span class="r_infotit">출발지</span>${rec.ttx_dep}</p>
                                <p class="r_info_text"><span class="r_infotit">목적지</span> ${rec.ttx_arr}</p>
                              </div>
                            </a>
                          </div>
                        </c:forEach>
                          ...
                          <c:when test="${!empty taxiHistory}">
                            ...
                            <c:forEach var="taxi" items="${taxiHistory}">
                              <tr class="historyLink">
                                ...
                                <td><a
                                    href="${contextPath}/history/viewTaxiRes.do?tx_res_code=${taxi.tx_res_code}">${taxi.tx_res_code}</a>
                                </td>
                                <td><a
                                    href="${contextPath}/history/viewTaxiRes.do?tx_res_code=${taxi.tx_res_code}">${taxi.tx_dep}</a>
                                </td>
                                <td><a href="${contextPath}/history/viewTaxiRes.do?tx_res_code=${taxi.tx_res_code}">
                                    ${taxi.tx_arr}</a></td>
                                <td><a
                                    href="${contextPath}/history/viewTaxiRes.do?tx_res_code=${taxi.tx_res_code}">${taxi.tx_number}</a>
                                </td>
                                <td><a
                                    href="${contextPath}/history/viewTaxiRes.do?tx_res_code=${taxi.tx_res_code}">${taxi.tx_reserv_day}</a>
                                </td>
