<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" isELIgnored="false" %>
  <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %> <!-- core라이브러리를 c라는 접두사를 따와서 쓰겠다 -->
    ...
    <% request.setCharacterEncoding("utf-8"); %>
      ...
      <c:set var="qList" value="${qMap.qList }" />
      <c:set var="totQ" value="${qMap.totalQ }" />
      <c:set var="section" value="${qMap.section }" />
      <c:set var="pageNum" value="${qMap.pageNum }" />
      <!DOCTYPE html>
      <html>
      ...
      <body>
        <div id="container_sub">
          ...
          <h2 class="pagetitlte">나의 질문 관리</h2>
                    ...
                    <tbody>
                      ...
                      <c:choose>
                        <c:when test="${empty qList}">
                          <td colspan="5">등록한 질문글이 없습니다.</td>
                        </c:when>
                        <c:when test="${not empty qList}">
                          <c:forEach var="qus" items="${ qList }">
                            <tr>
