<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" isELIgnored="false" %>
  <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
    ...
    <% request.setCharacterEncoding("utf-8"); %>
      ...
      <head>
        <meta charset="UTF-8">
        <title>회원 정보 일치 유무 페이지 </title>
        <link rel="stylesheet" type="text/css" href="${contextPath}/resources/css/reset.css" />
        <link rel="stylesheet" type="text/css" href="${contextPath}/resources/css/common.css" />
        <link rel="stylesheet" type="text/css" href="${contextPath}/resources/css/userVaild.css" />
          ...
          <section id="tab_Area">
            <div class="tabcontents">
              <div id="tabBox" class="tab-content ">
                <form action="${contextPath}/user_Page/isValid.do" id="check_form" method="post">
                  <div id="msg_box">
                    <img src="${contextPath}/resources/img/lock_icon.svg" alt="자물쇠이미지">
                    <p class="msg">회원님의 회원정보를 안전하게 보호하기 위해 비밀번호를 한번 더 확인해주세요. <br>
                      비밀번호가 타인에게 노출되지 않도록 주의해 주세요.</p>
                  </div>
                  <label for="edit_pwd">비밀번호</label>
                  <div id="input_box">
                    <input type="hidden" id="edit_id" name="user_id" value="${log_id}">
                    <input type="password" id="edit_pwd" name="user_pwd" placeholder="비밀번호 입력">
                  </div>
                  <div class="btn">
                    <a href=""><input form="check_form" type="button" value="취소" id="cancel_btn"></a>
                    <input form="check_form" type="submit" value="확인" id="pwd_confirm">
                  </div>
                </form>
              </div>
            </div>
