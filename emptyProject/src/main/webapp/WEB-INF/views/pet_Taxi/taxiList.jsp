            ...
            <script type="text/javascript">
              ...
              function setParent() {
                ...
                if (selectedTx_code === undefined) {
                  ...
                  let imgVar = "${contextPath}/resources/imgRepo/taxiImg/" + selectedRow.find('input[name="tx_code"]').val() + "/" + selectedRow.find('input[name="tx_img"]').val();
                    ...
                    <tbody>
                      <c:choose>
                        <c:when test="${empty taxiList}">
                          <td colspan="4">배차 가능한 택시가 없습니다.</td>
                        </c:when>
                        <c:when test="${not empty taxiList}">
                          <c:forEach var="tx" items="${taxiList}">
                            <tr>
                              ...
                              <td>
                                <input type="hidden" name="tx_code" value="${tx.tx_code}">
                              </td>
                              <td>
                                <input type="hidden" name="tx_img" value="${tx.tx_img}">
                              </td>
                              <td>
                                <input type="hidden" name="tx_intro" value="${tx.tx_intro}">
                              </td>
