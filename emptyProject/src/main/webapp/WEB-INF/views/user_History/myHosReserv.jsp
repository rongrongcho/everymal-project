  ...
  <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %> <!-- core라이브러리를 c라는 접두사를 따와서 쓰겠다 -->
    ...
    <% request.setCharacterEncoding("utf-8"); %>
      ...
      <head>
        ...
        <link rel="stylesheet" href="${contextPath}/resources/css/my_hos_res.css" />
      </head>
                      ...
                      <c:when test="${not empty recentList}">
                        <c:forEach var="rec" items="${recentList}">
                          <div class="reservBox">
                            ...
                            <a href="${contextPath}/user_History/userResView.do?res_code=${rec.res_code}"
                              style="display: flex">
                              <img
                                src="${contextPath}/resources/imgRepo/hos_images/thumbnail/${rec.hos_code}/${rec.hos_thum}"
                                alt="이미지1" />
                              <div class="r_smallBox" style="display: flex; flex-direction: column">
                                <p class="r_info_text"><span class="r_infotit">예약 일시</span>${rec.res_date}</p>
                                <p class="r_info_text"><span class="r_infotit">예약 접수</span>${rec.receipt_date}</p>
                                <p class="r_info_text"><span class="r_infotit">예약 병원</span> ${rec.hos_name}</p>
                            ...
                            <c:when test="${!empty historyList}">
                              ...
                              <c:forEach var="his" items="${historyList}">
                                ...
                                <tr class="historyLink">
                                  ...
                                  <td><a
                                      href="${contextPath}/user_History/userResView.do?res_code=${his.res_code}">${his.res_code}</a>
                                  </td>
                                  <td><a
                                      href="${contextPath}/user_History/userResView.do?res_code=${his.res_code}">${his.receipt_date}</a>
                                  </td>
                                  <td><a
                                      href="${contextPath}/user_History/userResView.do?res_code=${his.res_code}">${his.hos_name}
                                    </a> </td>
                                  <td><a
                                      href="${contextPath}/user_History/userResView.do?res_code=${his.res_code}">${his.res_date}</a>
                                  </td>
